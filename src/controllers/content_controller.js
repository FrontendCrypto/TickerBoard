...
export default class extends Controller {
    static targets = ["price", "change", "changeDirection", "quantity", "currencyBalance", "name", "icon"];
    static values = {
        change: Number
    }
    connect() {
    ...
    onUpdateContent() {
        document.addEventListener('update-content', (event) => {
            ...
            this.quantityTarget.textContent = this.formatCurrency(quantity, ticker);

            this.currencyBalanceTarget.textContent = this.formatCurrency(currencyBalance, 'EUR');
    ...
    formatCurrency(number, currency) {
        const formattedCurrency = new Intl.NumberFormat('es-ES', { style: 'currency', currency, minimumFractionDigits: 2 }).format(number)
